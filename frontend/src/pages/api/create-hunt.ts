...
export default async function handler(req: any, res: any) {
  ...
  //   console.log("enc key:", process.env.ENCRYPTION_KEY!); // Buffer.from(process.env.ENCRYPTION_KEY!)
  ...
  const target = ethers.utils.arrayify(ethers.utils.hexlify(encryptedImageString)); // convert to bytes32
  ...
  await hzhContract.addHunt(huntId, name, prize, endTime, target).then((t: any) => t.wait());

  // Send the grayscale values as the response
  res.status(200).json(imageArray.length);
  //   } catch (error: any) {
